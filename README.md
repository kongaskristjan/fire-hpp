...
Fire for C++, inspired by [python-fire](https://github.com/google/python-fire), is a library that creates a command line interface from a function signature. Here's a program that adds two numbers from command line:
 ```
#include <iostream>
#include <fire.hpp>
...
2. check for errors detected by `parse()`, print them and return
3. check for `-h` or `--help`, print the help message (hopefully) generated by the library and return
4. for each argument:
    1. get argument from map and convert to the right type
    2. try/catch for errors in conversion and return

That's a non-trivial amount of boilerplate, especially for simple scripts. It's possible to reduce boilerplate by skipping error checking and help messages, however that comes with significant usability cost. Also, many libraries don't help at all with the conversion step and this is usually skipped in examples too.
